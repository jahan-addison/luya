...
int Collide(Contact* contacts, Body const* bodyA, Body const* bodyB)
{
    ...
    np = ClipSegmentToLine(
        clipPoints2, clipPoints1, sideNormal, posSide, posEdge);
    ...
    int numContacts = 0;
    for (int i = 0; i < 2; ++i) {
        float cSeparation = Dot(frontNormal, clipPoints2[i].v) - front;

        if (cSeparation <= 0) {
            contacts[numContacts].separation = cSeparation;
            contacts[numContacts].normal = normal;
            // slide contact point onto reference face (easy to cull)
            contacts[numContacts].position =
                clipPoints2[i].v - cSeparation * frontNormal;
